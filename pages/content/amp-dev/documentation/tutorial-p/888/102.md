...
{% set markdown_content = g.doc('/content/amp-dev/documentation/tutorial-p/102/a101.md').content %}
...
<div class="ap-m-biggy-aside">
  {% do doc.styles.addCssFile('css/components/molecules/code-snippet.css') %}
  {% set js_snippet = g.doc('/shared/data/json-ld.js').content %}
  {% set markdown = doc.pod.markdown %}
  {{ markdown.convert('```javascript\n' + js_snippet + '\n```')|safe }}
</div>
